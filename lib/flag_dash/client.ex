defmodule FlagDash.Client do
  ...
  def config(client, key, default \\ nil) do
    with :miss <- cached(client, {:config, key}),
      ...
      value = Map.get(Map.get(config, "value", %{}), "value", default)
  ...
  @doc """
  Fetches a decrypted secret by key.

  Deliberately unlike `config/3`: the value is never cached, no default is
  substituted, and the error is returned rather than swallowed. Returning a
  stale or default credential is worse than failing loudly — a rotated key must
  take effect on the next call, and a silent fallback would send the wrong
  credential to a payment provider or a database.

  Requires an `sk_` key carrying the `secrets:read` scope.
  """
  @spec get_secret(t(), String.t()) :: {:ok, map()} | {:error, term()}
  def get_secret(client, key) do
    case request(client, :get, "/server/secrets/#{encode_segment(key)}") do
      {:ok, %{"secret" => %{"value" => _} = secret}} -> {:ok, secret}
      {:ok, _} -> {:error, :secret_missing_value}
      {:error, reason} -> {:error, reason}
      other -> {:error, other}
    end
  end

  @doc "Just the decrypted value. Same failure semantics as `get_secret/2`."
  @spec secret(t(), String.t()) :: {:ok, term()} | {:error, term()}
  def secret(client, key) do
    with {:ok, %{"value" => value}} <- get_secret(client, key), do: {:ok, value}
  end

  @doc "Explicit backend resolution; requires configs:read and secrets:read. Never cached or defaulted."
  def resolve_config(client, key) do
    with {:ok, body} <-
           request(client, :post, "/server/configs/#{encode_segment(key)}/resolve", json: %{}) do
      case body do
        %{"config" => config} when is_map(config) -> {:ok, config}
        _ -> {:error, :resolved_config_missing}
      end
    end
  end

  @doc "Evaluate a release and resolve its credentials. Never cached or defaulted."
  def resolve_ai_release(client, key, user_id \\ "anonymous") do
    with {:ok, body} <-
           request(client, :post, "/server/ai-config-releases/#{encode_segment(key)}/resolve",
             json: %{user_id: user_id}
           ) do
      case body do
        %{"ai_config" => config} when is_map(config) -> {:ok, config}
        _ -> {:error, :resolved_release_missing}
      end
    end
  end

  @doc "Evaluates a release without caching or resolving secret references."
  def ai_config_release(client, key, user_id \\ "anonymous") do
    with {:ok, %{"ai_config" => config}} <-
           request(client, :get, "/ai-config-releases/#{encode_segment(key)}",
